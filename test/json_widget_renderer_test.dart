...
void main() {
  group('JsonWidgetRenderer', () {
    ...
    testWidgets('emoji icon renders Text', (tester) async {
      await tester.pumpWidget(
        buildTree({'type': 'icon', 'icon': '🚀', 'size': 24}),
      );
      expect(find.text('🚀'), findsOneWidget);
    });

    testWidgets('named icon renders Icon widget', (tester) async {
      await tester.pumpWidget(buildTree({'type': 'icon', 'icon': 'star'}));
    ...
    testWidgets('textField registers value with field registry', (
      tester,
    ) async {
    ...
    testWidgets('switch fires onChanged event', (tester) async {
      await tester.pumpWidget(
        buildTree({'type': 'switch', 'value': false, 'onChange': 'toggle'}),
      );
    ...
    testWidgets('checkbox fires onChanged event', (tester) async {
      await tester.pumpWidget(
        buildTree({'type': 'checkbox', 'value': false, 'onChange': 'check'}),
      );
    ...
    testWidgets('slider fires onChanged event', (tester) async {
      await tester.pumpWidget(
        buildTree({'type': 'slider', 'value': 0.5, 'onChange': 'slide'}),
      );
    ...
    testWidgets('dropdown renders items and fires event', (tester) async {
      ...
      await tester.pumpAndSettle();
      expect(
        events.any((e) => e.$1 == 'select' && e.$2['value'] == 'c'),
        isTrue,
      );
    });
    ...
    testWidgets('markdown renders MarkdownBody', (tester) async {
      await tester.pumpWidget(
        buildTree({'type': 'markdown', 'data': '# Title'}),
      );
    ...
    testWidgets('chip fires onPressed', (tester) async {
      await tester.pumpWidget(
        buildTree({'type': 'chip', 'label': 'tag', 'onTap': 'filter'}),
      );
      ...
      await tester.pumpWidget(
        buildTree({
          ...
          'children': [
            ...
            {'type': 'textButton', 'text': 'Text', 'onTap': 't'},
          ],
        }),
      );
    ...
    testWidgets('stack with positioned child', (tester) async {
      ...
      expect(
        find.byWidgetPredicate(
          (w) => w is Stack && w.children.any((child) => child is Positioned),
        ),
        findsOneWidget,
      );
    ...
    testWidgets('decoration accepts CSS-style borderRadius list', (
      tester,
    ) async {
      await tester.pumpWidget(
        buildTree({
      ...
      expect(br.topLeft, const Radius.circular(14));
      expect(br.topRight, const Radius.circular(14));
      expect(br.bottomRight, Radius.zero);
      expect(br.bottomLeft, Radius.zero);
    });

    testWidgets('decoration accepts two-value borderRadius list', (
      tester,
    ) async {
      await tester.pumpWidget(
        buildTree({
      ...
      final decoration = container.decoration as BoxDecoration;
      final br = decoration.borderRadius as BorderRadius;
      expect(br.topLeft, const Radius.circular(10));
      expect(br.bottomRight, const Radius.circular(10));
      expect(br.topRight, const Radius.circular(4));
      expect(br.bottomLeft, const Radius.circular(4));
    });

    testWidgets('numeric props tolerate strings, lists and garbage', (
      tester,
    ) async {
      await tester.pumpWidget(
        buildTree({
    ...
    testWidgets('text supports textShadows and uppercase transform', (
      tester,
    ) async {
      await tester.pumpWidget(
        buildTree({
    ...
    testWidgets('path renders CustomPaint and respects progress/color', (
      tester,
    ) async {
      await tester.pumpWidget(
        buildTree({
    ...
    testWidgets('custom builder for video node', (tester) async {
      final customRenderer = JsonWidgetRenderer(
        onEvent: (id, payload) => events.add((id, payload)),
        customBuilders: {
          'video': (context, m) => Text('custom video: ${m['src']}'),
        },
      );
      await tester.pumpWidget(
        MaterialApp(
          home: Scaffold(
            body: customRenderer.build({'type': 'video', 'src': 'movie.mp4'}),
          ),
        ),
      );
    ...
    testWidgets('unregistered audio node renders placeholder icon', (
      tester,
    ) async {
      await tester.pumpWidget(
        buildTree({'type': 'audio', 'label': 'My Track'}),
      );
      expect(find.byType(Icon), findsOneWidget);
