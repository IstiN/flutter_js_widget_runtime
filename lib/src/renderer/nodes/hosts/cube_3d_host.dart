...
/// views.
Js3dHost createCube3dHost() => Cube3dHost.instance;
...
class Cube3dController extends Js3dController {
  ...
  void _apply(Js3dCommand command) {
    ...
    switch (command.kind) {
      ...
      case 'setTransform':
        ...
        if (existing != null) {
          js3dReadVec3(payload['position'] as List?)?.copyInto(existing.position);
          js3dReadVec3(payload['rotation'] as List?)?.copyInto(existing.rotation);
          js3dReadVec3(payload['scale'] as List?)?.copyInto(existing.scale);
  ...
  void _addModel({
    ...
    final parsedColor = js3dParseColor(color, Colors.blue);
    final mesh = src == null ? _primitiveMesh(primitive, parsedColor) : null;
    final obj = cube.Object(
      position: js3dReadVec3(position) ?? Vector3.zero(),
      rotation: js3dReadVec3(rotation) ?? Vector3.zero(),
      scale: js3dReadVec3(scale) ?? Vector3.all(1),
      mesh: mesh,
  ...
  void _applyMaterialColor(cube.Object obj, Color color) {
    // flutter_cube material channels are in the 0-1 range, matching the
    // Color.r/g/b accessors.
    final v = Vector3(color.r, color.g, color.b);
  ...
  void _applyCamera(Map<String, dynamic>? cam) {
    ...
    js3dReadVec3(cam['position'] as List?)?.copyInto(scene.camera.position);
    js3dReadVec3(cam['target'] as List?)?.copyInto(scene.camera.target);
    js3dReadVec3(cam['up'] as List?)?.copyInto(scene.camera.up);
  ...
  void _applyLight(Map<String, dynamic>? light) {
    ...
    js3dReadVec3(light['position'] as List?)?.copyInto(scene.light.position);
    final color = js3dParseColor(light['color'] as String? ?? '#ffffff', Colors.blue);
