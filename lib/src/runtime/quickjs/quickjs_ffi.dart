...
/// path. It is an implementation detail of the opt-in QuickJS FFI engine
/// backend (`js_widget_engine_quickjs.dart`).
library;
...
///
/// Looks in (1) the `JSR_QUICKJS_LIB` env var, (2) relative to
/// [Platform.script] (works for `dart run`/compiled binaries), and (3) the
/// current working directory (works for `flutter test`/`dart test`). Returns
/// the first existing candidate so the same code resolves under both
/// invocation styles.
