...
- `jsr.ease.*` — easing helpers (`linear`, `easeIn`, `easeOut`, `easeInOut`, `easeInOutCubic`, `easeInOutQuart`, `easeOutExpo`, `bounce`/`easeOutBounce`, `elastic`/`easeOutElastic`, `backIn`, `backOut`/`easeOutBack`, `cubicBezier(x1, y1, x2, y2)` factory).
- `jsr.motion.*` — declarative motion helpers, all times in **ms**: `tween(ms, startMs, durMs, from, to, easing?)` (clamped to from/to outside the window), `mapRange(v, inMin, inMax, outMin, outMax, easing?)` (input clamped), `clamp(v, min, max)`, `wave(ms, periodMs, amplitude, phase?)` (zero-centered sine, phase in radians). `easing?` accepts a function or a `jsr.ease` name string (default `linear`). Agreed with the yoclip Motion-Canvas port — yoclip converts its frame units to ms in its own wrapper.
