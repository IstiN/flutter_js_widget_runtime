...
(function() {
  // Stroked icons need an explicit stroke color — SVG default is
  // stroke:none, and the renderer's srcIn tint can only recolor pixels the
  // artwork actually paints.
  var SVG = {
  ...
  jsr.scene3d.addModel(sceneId, {
    ...
    // The anny body model ships with a skin-tone texture only; tint it to a
    // saturated violet so the coach reads as a stylized mannequin (accent2
    // multiplied by bright skin washes out to near-white).
    color: '#8b5cf6'
  });
  sceneReady = true;
